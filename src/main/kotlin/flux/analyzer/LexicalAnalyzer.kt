...
class LexicalAnalyzer constructor(private val source: File,
    ...
    private fun analyzeLine(line: List<String>, indexLine: Int) {
        line.forEachIndexed { indexWord, token -> analyzeToken(token, Location(indexWord + 1, indexLine)) }
    }

    private fun analyzeToken(token: String, location: Location) {
        when {
            isTokenKeyWord(token) ->
                checkSingleQuotesToken(token, location).also { saveKeyTokenToInternalForm(it) }
